...
def audio_play(mp3, volume = None, wait = False):
    global __name
    audio_stop()
    winmm = instance()
    __name = winmm.open(mp3)
    if not __name:
        return False
    if volume is not None:
        winmm.set_volume(__name, volume)
    winmm.play(__name, wait = wait)
    return True
...
if __name__ == '__main__':
    ...
    def test2():
        ...
        return 0
    def test3():
        audio_play('e:/english/resource/audio/h/hello.mp3', wait = False)
        time.sleep(0.3)
        audio_play('e:/english/resource/audio/h/habitant.mp3', wait = True)
        # time.sleep(3)
        return 0
